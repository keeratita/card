// Allowed card brands for XSS prevention
const ALLOWED_BRANDS = new Set(['visa', 'mastercard', 'amex', 'jcb', 'discover', 'diners']);
...
const BRAND_LOGO_SVGS: Record<string, string> = {
  ...
  discover: `<svg viewBox="0 0 24 24"><rect width="24" height="24" rx="3" fill="#1a1a1a"/><text x="12" y="16" font-size="5" text-anchor="middle" fill="#ff6000" font-family="Arial, sans-serif" font-weight="bold">discover</text></svg>`,
  diners: `<svg viewBox="0 0 24 24"><rect width="24" height="24" rx="3" fill="#003087"/><text x="12" y="14" font-size="3.5" text-anchor="middle" fill="#fff" font-family="Arial, sans-serif" font-weight="bold" letter-spacing="0.5">Diners Club</text></svg>`,
};

const DEFAULT_CARD_LOGO_SVG = `<svg viewBox="0 0 24 24"><path fill="#8e8e93" d="M12,2C6.48,2,2,6.48,2,12s4.48,10,10,10s10-4.48,10-10S17.52,2,12,2z M12,20c-4.41,0-8-3.59-8-8 s3.59-8,8-8s8,3.59,8,8S16.41,20,12,20z"/><path fill="#2c2c2e" d="M7,9h10v1H7V9z M7,11h7v1H7V11z"/></svg>`;

export function getCardLogoSvg(brand: string): string {
  // Validate brand against allowlist to prevent XSS
  if (typeof brand !== 'string' || !ALLOWED_BRANDS.has(brand)) {
    return DEFAULT_CARD_LOGO_SVG;
  }
  return BRAND_LOGO_SVGS[brand];
}